...
function handleActions(player, teleport) {
    if (world.scoreboard.getObjective("teleportTickCount")?.getScore(player) === 0) {
        ...
        player.teleport(teleport.toCoordinates, { "rotation": teleport.rotation });
    }
    player.runCommand("title @s times 10 100 20");
    const level = world.scoreboard.getObjective("info")?.getScore("level") || 0;
    if (level >= 1 && level <= 5) {
        player.runCommand("titleraw @s subtitle {\"rawtext\": [" +
    ...
    if (world.scoreboard.getObjective("teleportTickCount")?.getScore(player) === 15) {
        player.runCommand("titleraw @s title {\"rawtext\": [" +
            "{\"translate\": \"teleport.ng1:" +
            teleport.tag.substring(2, teleport.tag.length - 2) + "\"}]}");
    }
...
function handleTags() {
    ...
    for (let player of world.getAllPlayers()) {
        ...
        for (const teleport of teleportInfo) {
            ...
            if (positionInAreCheck(player.location, teleport.fromCoordinates[0], teleport.fromCoordinates[1])) {
