...
function handleActions(player: Player, tagIndex: number) {
    if (world.scoreboard.getObjective("teleportTickCount")?.getScore(player) === 0) {
        ...
        player.teleport( // Teleport the player
            teleportInfo,
            {"rotation": toCoordinates[tagIndex][1]}
        );
    }
    ...
    if (world.scoreboard.getObjective("teleportTickCount")?.getScore(player) === 15) {
        ...
        player.runCommand( // Show title
            ...
            teleportTags[tagIndex].substring(
                2, teleportTags[tagIndex].length - 2) + "\"}]}"
        );
    }
...
function handleTags() {
    ...
    const notAddedPlayers = world.getPlayers({"scoreOptions": [{ // Detect players not in scoreboard
        "exclude": true,
        "objective":
        "teleportTickCount",
        "minScore": 0,
    ...
    const allPlayers: Player[] = world.getAllPlayers(); // Get all players
    ...
    for (let players of allPlayers) {
        ...
        for ( // Do for all teleports
            let teleports_index: number = 0;
            teleports_index < teleportTags.length;
            teleports_index++
        ) {
            ...
            if (positionInAreCheck ( // If player in teleport area
                players.location,
                fromCoordinates[teleports_index][0],
                fromCoordinates[teleports_index][1]
            )) {
...
world.afterEvents.playerInteractWithEntity.subscribe(data => { // Detect interactions
